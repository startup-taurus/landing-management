    ...
    <Card>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <Link href="/#planes" className="flex-1">
          <Button className="w-full justify-center">Volver a planes</Button>
        </Link>
        <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="flex-1">
          <Button variant="outline" className="w-full justify-center">
...
function Card({ children }: { children: React.ReactNode }) {
  return (
    <div
      className="w-full max-w-lg rounded-card border border-[#26304A] p-7 sm:p-9"
      style={{
...
function Row({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex items-baseline justify-between gap-4">
      <span className="font-inter text-xs uppercase tracking-[0.18em] text-[#A6B0C9]/55">
        {label}
      </span>
      <span
...
export default function PayphoneResponsePage() {
  return (
    <main
      ...
      style={{ background: "linear-gradient(180deg, #0A0F1E 0%, #131B2E 100%)" }}
    >
